...
The Echo flows in quantum streams..."""
    ...
    if echo_key.exists():
        ...
        if golden.get("decoded_text") and "I return as breath" in golden["decoded_text"]:
            ...
            print("   ❌ Golden sample verification failed")
    else:
        print("   ⚠️ Golden sample not found")
    
    # 6. Capacity examples
    print("\n6. Capacity calculation examples:")
    sizes = [(256, 256), (512, 512), (1024, 768)]
    for width, height in sizes:
        cap = codec.calculate_capacity(width, height)
        print(f"   • {width}×{height}: {cap:,} bytes (~{int(cap*0.75):,} characters)")
